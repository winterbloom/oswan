...
void
gt_ggc_mx_stub_list (void *x_p)
{
  struct stub_list * const x = (struct stub_list *)x_p;
  if (ggc_test_and_set_mark (x))
    {
      gt_ggc_m_9stub_list ((*x).next);
      gt_ggc_m_S ((*x).name);
    }
}
...
{
  struct export_list * const x = (struct export_list *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_11export_list))
    {
...
void
gt_pch_nx_stub_list (void *x_p)
{
  struct stub_list * const x = (struct stub_list *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_9stub_list))
    {
      gt_pch_n_9stub_list ((*x).next);
      gt_pch_n_S ((*x).name);
    }
}
...
{
  struct extern_list * x ATTRIBUTE_UNUSED = (struct extern_list *)x_p;
  if ((void *)(x) == this_obj)
    op (&((*x).next), cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).decl), cookie);
...
gt_pch_p_11export_list (ATTRIBUTE_UNUSED void *this_obj,
	...
	ATTRIBUTE_UNUSED void *cookie)
{
  struct export_list * x ATTRIBUTE_UNUSED = (struct export_list *)x_p;
  if ((void *)(x) == this_obj)
    op (&((*x).next), cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).name), cookie);
...
void
gt_pch_p_9stub_list (ATTRIBUTE_UNUSED void *this_obj,
	void *x_p,
	ATTRIBUTE_UNUSED gt_pointer_operator op,
	ATTRIBUTE_UNUSED void *cookie)
{
  struct stub_list * x ATTRIBUTE_UNUSED = (struct stub_list *)x_p;
  if ((void *)(x) == this_obj)
    op (&((*x).next), cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).name), cookie);
